...
public class UserAccountRegistrationServiceTest {
	...
	public void attributeAccountInformationsSuccessTest() {
		...
		String account_reference_transaction = "pmb" + user.getEmail().substring(0, 1) + user.getEmail().substring(2, 5)
				+ user.getDisplayName().substring(2, 5) + "b";
	...
	public void attributeAccountInformationsSuccessWithAssertResultTest() {
		...
		userAccountInformations.setAccountReferenceTransaction(account_reference_transaction);
		// WHEN
		assertEquals(userAccountInformations.getAccountReferenceTransaction(),
				userAccountRegistrationService.attributeAccountInformations(user).getAccountReferenceTransaction());
		// THEN
	}
}
